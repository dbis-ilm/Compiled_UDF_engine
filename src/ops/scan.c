...
static void cast_and_fill(char **buffer, char *value, Type t) {
    
    switch (t)
    {
    case INT:
        *((long*)buffer) = atol(value);
        break;

    case DOUBLE:
        *((double*)buffer) = atof(value);
        break;
...
Operator *scan_build(char *filepath, char *delimiter, int num_cols, Type *types, int vectorsize) {
    Operator *op = operator_alloc(&scan_next, &scan_close, NULL, num_cols, types, "FileScan");
    ScanState *state;
